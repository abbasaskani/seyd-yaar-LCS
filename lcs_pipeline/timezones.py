...
def select_preset(config_raw: dict[str, Any], centroid_lon: float, centroid_lat: float, override_key: str | None = None) -> TimezoneDecision:
...
def choose_actual_time(candidates_utc, windows_utc, preset: TimePreset, auto_selected: bool, requested_nominal_local: datetime) -> TimeSelection:
    ...
    return TimeSelection(preset, auto_selected, requested_nominal_local, chosen, actual_local, status, windows_utc, requested_nominal_local.date().isoformat())
