...
def save_ftle_netcdf(out: FTLEOutputs, path: str | Path) -> Path:
    ...
    if out.persistence_3d is not None:
        data_vars["persistence_3d"] = (("x", "y"), out.persistence_3d)
    if out.persistence_5d is not None:
        data_vars["persistence_5d"] = (("x", "y"), out.persistence_5d)
    if out.balanced_composite is not None:
        data_vars["balanced_composite"] = (("x", "y"), out.balanced_composite)
    if out.physics_first_composite is not None:
        data_vars["physics_first_composite"] = (("x", "y"), out.physics_first_composite)
    ds = xr.Dataset(
        data_vars=data_vars,
        coords={"x": out.x_grid[:, 0], "y": out.y_grid[0, :]},
        attrs={"target_time": out.target_time, "u_variable": out.u_variable, "v_variable": out.v_variable, **(out.metadata or {})},
    )
    ds.to_netcdf(path)
    return path
...
def save_summary_json(out: FTLEOutputs, path: str | Path, extra: dict | None = None) -> Path:
    path = Path(path)
    payload = {"target_time": out.target_time, "u_variable": out.u_variable, "v_variable": out.v_variable, "hotspots": out.hotspots, "top_clusters": out.clusters[:10], "metadata": out.metadata or {}}
    if extra:
...
def save_field_layers_json(out: FTLEOutputs, path: str | Path) -> Path:
    ...
    payload = {
        ...
        "bbox": {"lon_min": float(np.nanmin(out.lon_grid)), "lon_max": float(np.nanmax(out.lon_grid)), "lat_min": float(np.nanmin(out.lat_grid)), "lat_max": float(np.nanmax(out.lat_grid))},
        "layers": {
